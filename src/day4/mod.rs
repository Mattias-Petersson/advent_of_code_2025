...
fn save_positions(grid: &[Vec<char>]) -> HashSet<(usize, usize)> {
...
fn check_neighbors(grid: &[Vec<char>], position: (usize, usize), size: (usize, usize)) -> bool {
    ...
    let row_start = curr_row.saturating_sub(1);
    let row_end = (curr_row + 1).min(no_rows - 1);
    let col_start = curr_col.saturating_sub(1);
    let col_end = (curr_col + 1).min(no_cols - 1);

    let neighbor_count = (row_start..=row_end)
        .flat_map(|i| (col_start..=col_end).map(move |j| (i, j)))
        .filter(|&(i, j)| !(i == curr_row && j == curr_col))
        .filter(|&(i, j)| grid[i][j] == '@')
        .count();
...
mod tests {
    ...
    fn test_against_example() {
        let file = File::open("src/day4/example_input.txt").unwrap();
